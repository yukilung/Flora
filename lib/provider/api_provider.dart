...
class ApiProvider extends ChangeNotifier {
  ...
  /// 0: start AI retraining
  /// 1: falsed to call API
  /// 2: already training
  Future<int> requestRetrain({Choice choice = Choice.Local}) async {
    ...
    try {
      ...
      if (response.statusCode == 202) {
        if (jsonResponse.containsKey('Task Id')) {
          ...
          /// server return null status
          /// 當前已有進行中的任務
          if (taskId == null) {
            return 2;
          }

          // task id 持久化, 用於之後檢測是否有進行中的任務
  ...
  Future<Map<String, dynamic>> browseTaskStatus() async {
    ...
    try {
      ...
      // CC: 如果手機玩玩下斷左, keep running
      final error = DioExceptions.fromDioError(e);
      debugPrint(error.messge);

      final String serverError = 'Server error';

      if (error.messge.substring(0, 12) == serverError) {
        result = {'error': true};
        _backgroundService(false);
        _training = false;
        notifyListeners();
      ...
      print('catch error, but keep running');
      return result;
    }
  }
